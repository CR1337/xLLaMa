...
def set_environment(gpu_ids: List[int]):
    ...
    dotenv.set_key(".env", "LOCAL", "1" if is_local else "0")
    if gpu_ids:
        dotenv.set_key(".env", "GPU0", str(gpu_ids[0]))
        dotenv.set_key(".env", "GPU1", str(gpu_ids[-1]))
    else:
        dotenv.set_key(".env", "GPU0", "0")
        dotenv.set_key(".env", "GPU1", "0")
